...
def dashboard():
    ...
    return render_template(
        "dashboard.html", username=session["username"], admin=session["admin"]
    )
...
def admin():
    ...
    if request.method == "POST":
        if request.form["type"] == "add_user":
            ...
            clear_tables("employee_scheduler.sqlite")
    users = get_all_users()
...
def shifts():
    ...
    schedule = get_schedule()
    return render_template(
        "shifts.html", schedule=schedule, users=users, username=session["username"]
    )
